...
| value       | 响应式值（表单数据来源）                  | `any`,使用`useReactiveForm` 或`useDecorateForm`会做相应式包装处理                                      | `—` | 是  |
...
| render2       | 自定义渲染函数（返回 ReactNode）                            | `(formItem: DyFormItem) => ReactNode` | `—`             | 否  |
...
[//]: # (| `render2`             | 自定义单元格渲染（优先级最高）               | `&#40;row: T, $index: number&#41; => ReactNode`               | `-`         | 否  |)
