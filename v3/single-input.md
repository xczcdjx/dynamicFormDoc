...
| `typeTools`  | `scope`       | `ScopeType`               | 自定义每一行的类型工具区（如 Array/Number 切换）。可使用 `toggleArray()` / `toggleNumber()` 等。      |
| `rowActions` | `scope`       | `ScopeType`               | 自定义每一行的操作区（如 + / -）。可使用 `addItem()` / `removeItem()`，并通过 `isLast` 控制最后一行才能新增等。 |
